...
echo "==================================="
echo "Personal Blog - D1 Database Setup"
echo "==================================="
echo ""
...
# Create D1 database
echo "Creating D1 database 'personal-blog-db'..."
DATABASE_OUTPUT=$(wrangler d1 create personal-blog-db 2>&1)

if echo "$DATABASE_OUTPUT" | grep -q "already exists"; then
    echo "ℹ️  Database already exists"
    # Extract database ID from list command
    DATABASE_ID=$(wrangler d1 list | grep "personal-blog-db" | awk '{print $2}')
else
